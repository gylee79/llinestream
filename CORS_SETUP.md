...
이 파일은 썸네일 및 비디오 업로드 기능이 정상적으로 동작하는 데 필요한 Firebase Storage의 CORS(Cross-Origin Resource Sharing) 정책을 설정하는 방법을 안내합니다. 이 작업은 **Firebase Storage 서비스를 활성화**하고 웹 앱에서의 파일 접근을 허용하는 중요한 과정입니다.
...
*   `gcloud storage buckets update`: Google Cloud Storage 버킷의 설정을 업데이트하는 명령어입니다. 이 명령어를 처음 실행하면 Storage API가 자동으로 활성화될 수 있습니다.

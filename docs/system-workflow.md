# [공식] LlineStream 비디오 시스템 워크플로우 (v8.0 - 단일 함수 상태 머신)
...
- **명확한 오류 추적**: `status.error`와 `ai.error` 필드를 통해 어느 단계에서 어떤 원인으로 문제가 발생했는지 명확하게 파악할 수 있습니다. 예를 들어, 비디오 인코딩에서 문제가 발생하면 `status.error.step`에 `'transcode'`가 기록됩니다.

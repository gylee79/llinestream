...
`buffering-seek`와 `license-expired` 상태가 추가되어, 모든 재생 시나리오에 대응합니다.

```typescript
type PlayerState =
  | 'idle'
  | 'requesting-key'
  | 'downloading'      // Offset Map 구성 또는 초기 청크 다운로드
  | 'decrypting'       // 수신된 청크 복호화 중 (단계별 UI)
  | 'buffering-seek'   // 탐색 후 필요한 청크 버퍼링 중
  | 'ready'            // 재생 가능 (버퍼에 데이터 충분)
  | 'playing'
  | 'paused'
  | 'recovering'       // 청크 재시도 또는 키/URL 재발급 중

...
-   **안정성 확보:**
    -   **Seek 안정성 (v5.1.6):** 현재 구현은 안정성을 최우선으로 하여 전체 파일을 다운로드한 후 재생합니다. 하지만 Chunked 구조 덕분에, 향후 **부분 다운로드 및 즉시 재생(true streaming seek)** 기능으로 쉽게 확장할 수 있는 기반이 마련되었습니다. 현재는 전체 파일의 무결성이 보장된 후 재생이 시작되므로, 재생 중 탐색은 매우 안정적입니다.
...
4.  **AI 분석 및 암호화 병렬 처리 (`functions/src/index.ts`)**
    ...
    *   **작업 A: AI 콘텐츠 분석 (`runAiAnalysis`):**
        ...
        *   **상태 분리 (v5.1.5):** 이 작업이 실패하더라도 비디오 암호화 및 재생에는 영향을 주지 않습니다. `aiProcessingStatus` 필드로 상태를 별도 관리하며, **관리자 페이지에서 재시도할 수 있습니다.**

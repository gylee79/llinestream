...
import { deleteChatLog } from '@/lib/actions/chat-actions';
...
function ChatLogViewer() {
  ...
  const { data: chatLogs, isLoading: chatsLoading } = useCollection<ChatLog>(chatsQuery);

  const isLoading = episodesLoading || usersLoading || chatsLoading;
                ...
                {isLoading ? (
                  ...
                  chatLogs.map((log) => (
                      <TableRow key={log.id}>
                          ...
                          <TableCell className="text-sm">{getEpisodeTitle(log.episodeId)}</TableCell>
                          <TableCell className="text-sm">{log.question}</TableCell>
                          <TableCell className="text-sm text-muted-foreground">{log.answer}</TableCell>
                              ...
                              <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                      <AlertDialogHeader>
                                          ...
                                          <AlertDialogDescription>
                                              이 채팅 기록을 영구적으로 삭제합니다. 이 작업은 되돌릴 수 없습니다.
                                          </AlertDialogDescription>
                                      </AlertDialogHeader>
                                      <AlertDialogFooter>
